...
import {getPersonInfo, getUserInfo} from "../../api/user";
...
import UserCenterContext, {UserCenterProvider} from "../../context/UserCenterContext";

const UserCenterScreen = ({route}) => {
  ...
  const reqGetUserInfo = () => {
    if(!userId) return
    getPersonInfo(userId).then(async res => {
      await AsyncStorage.setItem("searchPerson", JSON.stringify(res.data))
    ...
    <UserCenterProvider va>
      <View>
        ...
        {userInfo  && <View style={{height: Dimensions.get('screen').height - 300}}>
          <UserPost userInfo={userInfo}></UserPost>
        </View>}
      </View>
    </UserCenterProvider>
